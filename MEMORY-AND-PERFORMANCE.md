...
| Area | Result |
| --- | --- |
| Runtime (1.3.0) | Real subscriptions exist (MessageHub, lifecycle behavior, navigators). Leak tests cover VM / command / weak `CanExecuteChanged` + Button pop / weak hub / navigation pop. Chat lists should use `SnapshotCollection` + `AddLocal`, not `ReplaceRange` after appear. |
