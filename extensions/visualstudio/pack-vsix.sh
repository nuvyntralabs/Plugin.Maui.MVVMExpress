...
dotnet restore "$root/MvvmExpress.VisualStudio.csproj" --nologo
dotnet build "$root/MvvmExpress.VisualStudio.csproj" -c Release --nologo --no-restore \
  -p:CreateVsixContainer=false \
  -p:DeployExtension=false \
  -p:GeneratePkgDefFile=false

props="$root/obj/MvvmExpress.VisualStudio.csproj.nuget.g.props"
if [ ! -f "$props" ]; then
...
cp "$vsdk/inc/stdidcmd.h" "$vsdk/inc/vsshlids.h" "$root/MvvmExpressPackage.vsct" "$pack/"
(
  cd "$pack"
  if [[ "${OS:-}" == "Windows_NT" ]] || uname | grep -qiE 'mingw|msys|cygwin'; then
    ./VSCT.exe -nologo MvvmExpressPackage.vsct MvvmExpressPackage.cto
  elif command -v wine >/dev/null 2>&1; then
    WINEDEBUG=-all wine VSCT.exe -nologo MvvmExpressPackage.vsct MvvmExpressPackage.cto
  else
    echo "wine is required on macOS to compile MvvmExpressPackage.vsct. Install wine, or pack on Windows CI." >&2
    exit 1
  fi
)
...
rm -rf "$stage"
mkdir -p "$stage"
cp "$root/bin/Release/net472/NuvyntraLabs.MVVMExpress.VisualStudio.dll" "$stage/"
cp "$repo/LICENSE" "$stage/"

cat > "$stage/NuvyntraLabs.MVVMExpress.VisualStudio.pkgdef" <<'EOF'
[$RootKey$\Packages\{5f2c8a91-7e4b-4d16-9a3c-1b8e6f0d4c22}]
@="MvvmExpressPackage"
"InprocServer32"="$WinDir$\SYSTEM32\MSCOREE.DLL"
"Class"="NuvyntraLabs.MVVMExpress.VisualStudio.MvvmExpressPackage"
"CodeBase"="$PackageFolder$\NuvyntraLabs.MVVMExpress.VisualStudio.dll"
"AllowsBackgroundLoad"=dword:00000001

[$RootKey$\InstalledProducts\MVVMExpress]
"Package"="{5f2c8a91-7e4b-4d16-9a3c-1b8e6f0d4c22}"
"UseInterface"=dword:00000000
"ProductDetails"="Create MVVMExpress MAUI apps and pages from Plugin.Maui.MVVMExpress.Templates."
"PID"="__VERSION__"

[$RootKey$\AutoLoadPackages\{adfc4e64-0397-11d1-9f4e-00a0c911004f}]
"{5f2c8a91-7e4b-4d16-9a3c-1b8e6f0d4c22}"=dword:00000002

[$RootKey$\AutoLoadPackages\{f1536ef8-92ec-443c-9ed7-fdadf150da82}]
"{5f2c8a91-7e4b-4d16-9a3c-1b8e6f0d4c22}"=dword:00000002

[$RootKey$\Menus]
"{5f2c8a91-7e4b-4d16-9a3c-1b8e6f0d4c22}"=", Menus.ctmenu, 1"
EOF
python3 -c "
from pathlib import Path
import sys
...
<PackageManifest Version="2.0.0" xmlns="http://schemas.microsoft.com/developer/vsx-schema/2011">
  <Metadata>
    <Identity Id="nuvyntralabs.MVVMExpress.VisualStudio" Version="${version}" Language="en-US" Publisher="Nuvyntra Labs" />
    <DisplayName>MVVMExpress for Visual Studio</DisplayName>
    <Description>Install Plugin.Maui.MVVMExpress.Templates and create MVVMExpress MAUI apps and pages via dotnet new.</Description>
    <MoreInfo>https://github.com/nuvyntralabs/Plugin.Maui.MVVMExpress</MoreInfo>
    <License>LICENSE</License>
    <Tags>MAUI;MVVM;templates;dotnet new;mvvmexpress</Tags>
  </Metadata>
  <Installation>
  ...
  <Assets>
    <Asset Type="Microsoft.VisualStudio.VsPackage" Path="NuvyntraLabs.MVVMExpress.VisualStudio.pkgdef" />
  </Assets>
</PackageManifest>
EOF

cp "$pack/MvvmExpressPackage.cto" "$stage/"

if [ -z "$out_vsix" ]; then
  out_vsix="$root/../dist/nuvyntralabs.MVVMExpress.VisualStudio.${version}.vsix"
fi
...
description = (
    "Install Plugin.Maui.MVVMExpress.Templates and create MVVMExpress "
    "MAUI apps and pages via dotnet new."
)
payload = [
    "extension.vsixmanifest",
    "LICENSE",
    "MvvmExpressPackage.cto",
    "NuvyntraLabs.MVVMExpress.VisualStudio.dll",
    "NuvyntraLabs.MVVMExpress.VisualStudio.pkgdef",
]
extension_dir = r"[installdir]\Common7\IDE\Extensions\nuvyntralabs\MVVMExpress"
    ...
    "packages": [
        {
            ...
            "localizedResources": [
                {
                    "language": "en-US",
                    "title": "MVVMExpress for Visual Studio",
