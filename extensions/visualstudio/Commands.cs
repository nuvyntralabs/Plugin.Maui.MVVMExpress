...
{
    public static readonly Guid CommandSet = new("9d1e4b70-2c5a-4f83-b6d1-8e7a3c5f9012");
    ...
    private static async Task CreateAppAsync(AsyncPackage package)
    {
        ...
        var parent = Prompts.AskFolder("Create the MVVMExpress app in this folder");
        if (parent is null)
        {
            return;
        }
    ...
    private static async Task AddPageAsync(AsyncPackage package)
    {
        ...
        var outputDir = Prompts.AskFolder("Add the MVVMExpress page in this folder", startFolder);
        ...
        ShowMessage(
            package,
            $"Created {name} page in {outputDir}. Call services.Add{name}() and map the route in MauiProgram.",
